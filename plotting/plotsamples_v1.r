...
x <- try(load(args[ARG_RESULT]));
if (class(x) == 'try-error') {
   stop('There was an error when trying to load in the Rdata object.\nError message from R:\n');
   print(geterrmessage());
   return();
}
dataDir <- dirname(args[ARG_OUTPUTDIR]);
col <- args[ARG_COL];
classesName <- args[ARG_CLASSESNAME];
xlabelIndiv <- args[ARG_XLABELINDIV];
